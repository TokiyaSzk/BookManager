...
int main(void) {
    ...
    // 提取各个参数
    char name[256], password[256],con_header[256],con_name[256];
    ...
    if (strcmp(list.nodes[postion].bnum,"0")==0)
    {
    ...
    fprintf(standard,"%s %s %s %s %s %s", con_header,name,password,con_name,list.nodes[postion].bnum,list.nodes[postion].outnum);
    fclose(standard);
    // printf("Content-Type: text/html\n\n");
    // printf("<html><body>\n");
    ...
    // 打开 Python 脚本文件
    fp = popen("/usr/bin/python3 /Applications/AMPPS/www/cgi-bin/borrow.py", "r");
    if (fp == NULL) {
        printf("Failed to run command\n" );
        exit(1);
    }
