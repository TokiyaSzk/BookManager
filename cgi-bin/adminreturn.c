...
int main(void) {
    ...
    // 提取各个参数
    char name[256], password[256],con_header[256],con_name[256],con_rname[256];
    get_param(decoded_buffer, "name", name);
    get_param(decoded_buffer, "password", password);
    get_param(decoded_buffer, "identity", con_header);
    get_param(decoded_buffer, "rname", con_rname);
    get_param(decoded_buffer, "bname",con_name);

    int postion;
    postion= node_find(&list,con_name);
    FILE *standard ;
    standard = fopen("/Applications/AMPPS/www/cgi-bin/standard.txt", "w");

    fprintf(standard,"%s %s %s %s %s %s %s", con_header,name,password,con_name,list.nodes[postion].bnum,list.nodes[postion].outnum,con_rname);
    fclose(standard);
    ...
    // 打开 Python 脚本文件
    fp = popen("/usr/bin/python3 /Applications/AMPPS/www/cgi-bin/returnbook.py", "r");
    if (fp == NULL) {
        printf("Failed to run command\n" );
        exit(1);
    }
