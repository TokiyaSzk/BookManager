...
int main(void) {
    ...
    // 提取各个参数
    char name[256], password[256],con_header[256],con_name[256],con_bnum[256],con_outnum[256];
    ...
    get_param(decoded_buffer, "outnum",con_outnum);

    FILE *standard ;
    standard = fopen("/Applications/AMPPS/www/cgi-bin/standard.txt", "w");
    fprintf(standard,"%s %s %s %s %s %s",con_header,name,password,con_name,con_bnum,con_outnum);
    fclose(standard);
